...
class UniswapV3EthereumPoolingPositionProvider(
    private val uniswapV3EthereumPoolingMarketProvider: UniswapV3EthereumPoolingMarketProvider,
    blockchainGatewayProvider: BlockchainGatewayProvider,
    val erC20Resource: ERC20Resource
) : PoolingPositionProvider() {

    val blockchainGateway = blockchainGatewayProvider.getGateway(Network.ETHEREUM)
        ...
        positionsForUser.filter {
            ...
            async {
                try {
                    ...
                    val token0 = erC20Resource.getTokenInformation(Network.ETHEREUM, poolContract.token0())
                    val token1 = erC20Resource.getTokenInformation(Network.ETHEREUM, poolContract.token1())
                    ...
                    val market = uniswapV3EthereumPoolingMarketProvider.toMarket(poolContract)

                    val totalToken0Usd = if (userTokens0 > BigDecimal.ZERO) {
                        uniswapV3EthereumPoolingMarketProvider.getPriceResource()
                            .calculatePrice(
                                PriceRequest(
                                    token0.address,
                                    Network.ETHEREUM,
                    ...
                    val totalToken1Usd = if (userTokens1 > BigDecimal.ZERO) {
                        uniswapV3EthereumPoolingMarketProvider.getPriceResource().calculatePrice(
                            PriceRequest(
                                token1.address,
                                Network.ETHEREUM,
                                userTokens1
    ...
    fun calculateAmount(
        ...
        return if (tokenPosition == 0) {
            (liquidity.toBigDecimal() * POW_96.toBigDecimal() * (sqrt.third.toBigDecimal() - sqrt.first.toBigDecimal()) / ((sqrt.third.toBigDecimal() * sqrt.first.toBigDecimal())) / BigInteger.TEN.pow(
                decimals
            ).toBigDecimal())
        } else {
            liquidity.toBigDecimal() * (sqrt.first.toBigDecimal() - sqrt.second.toBigDecimal()) / POW_96.toBigDecimal() / BigInteger.TEN.pow(
                decimals
            ).toBigDecimal()
        }
    }
