...
class ERC20Repository(
    ...
    private lateinit var tokenList: List<io.defitrack.erc20.TokenInfo>
    ...
    private fun fetchFromTokenList(url: String) =
        runBlocking {
            ...
            objectMapper.readValue(
                result,
                io.defitrack.erc20.TokenListResponse::class.java
            )
        }.tokens.mapNotNull { entry ->
            Network.fromChainId(entry.chainId)?.let { network ->
                io.defitrack.erc20.TokenInfo(
                    name = entry.name,
                    network = network,
    ...
    fun allTokens(network: Network): List<io.defitrack.erc20.TokenInfo> {
        return tokenList.filter { it.network == network }.distinctBy {
            it.address.lowercase()
        }
    }

    fun getToken(network: Network, address: String): io.defitrack.erc20.TokenInfo? {
        if (address == "0x0") {
            return io.defitrack.erc20.ERC20Repository.Companion.NATIVE_WRAP_MAPPING[network]?.let {
                tokenList.find { info ->
