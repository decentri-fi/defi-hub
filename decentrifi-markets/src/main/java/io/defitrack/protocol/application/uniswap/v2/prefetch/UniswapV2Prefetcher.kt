...
class UniswapV2Prefetcher(
    ...
    val fetches: Deferred<List<PoolingMarketInformationDTO>> = lazyAsync {
        val asString: String =
            httpClient.get("https://raw.githubusercontent.com/decentri-fi/data/master/pre-fetches/uniswap/uniswap_v2.json")
                .body()
        objectMapper.readValue(asString, object : TypeReference<List<PoolingMarketInformationDTO>>() {})
    }

    suspend fun getPrefetches(network: Network): List<PoolingMarketInformationDTO> = withContext(Dispatchers.IO) {
        try {
